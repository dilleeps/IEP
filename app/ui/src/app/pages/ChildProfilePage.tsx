...
export function ChildProfilePage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showSuccess(t.childProfile.deleteSuccess.replace("{name}", child.name));
      logger.info("Child deleted", { childId: child.id });
    } catch (error) {
      logger.error("Error deleting child", { childId: child.id, error });
      showError(t.childProfile.deleteFailed);
    } finally {
      setDeletingId(null);
    }
  };
  ...
  if (children.length === 0) {
    return (
      <div className="space-y-6 p-4 md:p-6">
        <EmptyState
          icon={User}
          title={t.childProfile.noChildren}
          description={t.childProfile.noChildrenDesc}
          action={
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <PageHeader
        ...
        description={t.childProfile.subtitle}
        action={
          <Button onClick={() => navigate(config.routes.childProfileNew)}>
            <Plus className="mr-2 h-4 w-4" />
            {t.childProfile.addChild}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="flex items-center gap-1">
                      ...
                      {t.childProfile.age} {child.age} • {t.childProfile.grade} {child.grade}
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {child.disabilities.length > 3 && (
                      <span className="inline-flex items-center px-2 py-1 rounded-md bg-muted text-muted-foreground text-xs font-medium">
                        +{child.disabilities.length - 3} {t.childProfile.more}
                      </span>
                    )}
                  </div>
                </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              {t.childProfile.deleteTitle}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-left space-y-2">
              <p>
                {t.childProfile.deleteConfirm} <strong>{deleteDialog.child?.name}</strong>?
              </p>
              <p className="text-destructive">
                {t.childProfile.deleteWarning}
              </p>
              <ul className="list-disc list-inside space-y-1 text-sm">
                <li>{t.childProfile.deleteGoals}</li>
                <li>{t.childProfile.deleteProgress}</li>
                <li>{t.childProfile.deleteBehavior}</li>
                <li>{t.childProfile.deleteContacts}</li>
              </ul>
              <p className="font-semibold">{t.common.actionCannotBeUndone}</p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {t.childProfile.deletePermanently}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
