...
import { Upload, FolderOpen, UserPlus } from "lucide-react";
...
interface FileUploadStepProps {
  ...
  /** Called when user wants to create a new child profile from the IEP */
  onCreateNewChild?: () => Promise<string | null>;
...
export function FileUploadStep({
  ...
  const [isNewChild, setIsNewChild] = useState(() => children.length === 0);

  // Auto-select "New Child" when user has no existing children
  useEffect(() => {
    if (children.length === 0) {
      setIsNewChild(true);
      setChildId("");
    }
  }, [children.length, setChildId]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={isNewChild ? NEW_CHILD_SENTINEL : childId}
            onValueChange={(val) => {
              if (val === NEW_CHILD_SENTINEL) {
                setIsNewChild(true);
                setChildId("");
              } else {
                setIsNewChild(false);
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value={NEW_CHILD_SENTINEL}>
                <span className="flex items-center gap-2">
                  <UserPlus className="h-3.5 w-3.5" />
                  New Child (create from IEP)
                </span>
              </SelectItem>
            </SelectContent>
          </Select>
          {isNewChild && (
            <p className="text-xs text-muted-foreground">
              A new child profile will be created automatically from the uploaded IEP document.
            </p>
          )}
        </div>
          ...
          <TabsContent value="upload" className="mt-4 space-y-3">
            ...
            <Button
              onClick={async () => {
                ...
                if (isNewChild && onCreateNewChild) {
                  const newId = await onCreateNewChild();
                  if (newId) {
                    setChildId(newId);
                    setIsNewChild(false);
                    onNext(selectedFile, newId);
                  }
                } else if (childId) {
                  onNext(selectedFile, childId);
                }
              }}
              disabled={!selectedFile || (!childId && !isNewChild) || isProcessing}
              className="w-full"
            >
              {isNewChild ? 'Create Profile & Analyse' : 'Upload & Analyse'}
            </Button>
          </TabsContent>
