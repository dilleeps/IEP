...
interface AuthContextValue {
  ...
  login: (credentials: LoginCredentials) => Promise<{ needsOnboarding: boolean }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(
    async (credentials: LoginCredentials) => {
      try {
        ...
        setSession({
          ...session,
          requiresSetup: session.requiresSetup ?? false,
        });
  ...
  const value: AuthContextValue = {
    ...
    login: async (credentials: LoginCredentials) => {
      ...
      return { needsOnboarding: (session as any)?.needsOnboarding ?? false };
    },
    loginWithGoogle,
    register,
    logout,
