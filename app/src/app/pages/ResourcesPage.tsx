...
export function ResourcesPage() {
  ...
  const resourceCategories = [
    {
      title: t.resources.advocacyOrgs,
      icon: Users,
    ...
    {
      title: t.resources.educationalSupport,
      icon: School,
  ...
  return (
    <div className="space-y-10 p-4 md:p-6 pb-20">
      ...
      <div className="relative rounded-[32px] md:rounded-[40px] p-8 md:p-10 text-slate-900 dark:text-white overflow-hidden shadow-2xl bg-gradient-to-br from-slate-100 via-indigo-50 to-indigo-100 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950">
        ...
        <div className="relative z-10 max-w-2xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/60 dark:bg-white/10 backdrop-blur-md border border-white/40 dark:border-white/10 text-[10px] font-black uppercase tracking-widest text-indigo-700 dark:text-indigo-300">
            <BookOpen className="w-3 h-3" /> {t.resources.knowledgeBase}
          </div>
          <h2 className="text-3xl md:text-4xl font-black tracking-tight">{t.resources.title}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: ShieldAlert, title: t.resources.ideaRights, tag: t.resources.mustRead, color: "bg-amber-500", url: "https://sites.ed.gov/idea/parents-families/" },
          { icon: MessageCircle, title: t.resources.meetingScripts, tag: t.resources.advocacy, color: "bg-blue-500", url: "https://www.wrightslaw.com/info/advo.index.htm" },
          { icon: Video, title: t.resources.webinarSeries, tag: t.resources.training, color: "bg-purple-500", url: "https://www.copaa.org/page/webinars" },
        ].map((item, i) => (
          <a
            key={i}
            href={item.url}
          ...
          >
            ...
            <p className="text-muted-foreground text-xs mt-1 font-bold flex items-center gap-1">
              {t.resources.exploreGuide} <ArrowRight className="w-3 h-3" />
            </p>
          </a>
        ))}
      </div>
      ...
      <div className="bg-muted rounded-[32px] md:rounded-[40px] p-8 md:p-10 border border-border flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="flex-1 text-center md:text-left space-y-2">
          <h3 className="text-2xl font-black text-slate-900 dark:text-white">{t.resources.needLocalHelp}</h3>
          <p className="text-muted-foreground font-medium max-w-xl">
            {t.resources.localHelpDesc}
          </p>
        </div>
        <a
        ...
        >
          {t.resources.findStatePTI} <Search className="w-5 h-5" />
        </a>
      </div>
    </div>
  );
