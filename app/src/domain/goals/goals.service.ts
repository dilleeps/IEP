...
function mapBackendGoalToFrontend(backendGoal: BackendGoalResponse): Goal {
  ...
  // Parse numeric baseline/target from SMART criteria text, falling back to defaults
  const baselineNum = parseFloat(backendGoal.currentValue || "") || 0;
  const targetNum = parseFloat(backendGoal.targetValue || "") || 100;
  const progressPct = backendGoal.progressPercentage ?? 0;
  ...
  return {
    ...
    baselineLevel: baselineNum,
    baselineText: backendGoal.baseline || "",
    target: targetNum,
    targetLevel: targetNum,
    targetText: backendGoal.target || "",
    current: progressPct,
    currentLevel: progressPct,
    metric: "%",
    ...
    startDate: backendGoal.startDate || backendGoal.createdAt,
