...
function IEPAnalysisWorkflow() {
  ...
  const loadAndFinalize = useCallback(async (id: string, skipFinalize = false) => {
    try {
      ...
      if (skipFinalize) {
        // Check if the document was actually finalized (goals created in DB)
        // If extractionStatus is still 'pending_review', we must finalize
        const statusData = await getIEPService().getDocumentStatus(id);
        if (statusData.extractionStatus === 'finalized') {
          // Truly finalized — build result counts from extraction data
        ...
        // Not yet finalized — fall through to finalize
        addLog("Extraction not yet finalized — creating goals...");
      }
