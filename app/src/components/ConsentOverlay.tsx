...
export function ConsentOverlay() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <Card className="w-full max-w-3xl rounded-3xl shadow-2xl bg-background text-foreground border border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-2xl font-semibold text-foreground">Parent Consent Required</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-2xl border border-border bg-muted/40 p-4 max-h-[320px] overflow-auto text-sm leading-relaxed text-foreground">
            {termsText.split("\n").map((line, idx) => (
              <p key={idx} className={`mb-1.5 last:mb-0 ${line === "" ? "h-2" : ""}`}>
                {line}
              </p>
            ))}
            <hr className="my-4 border-border" />
            {privacyText.split("\n").map((line, idx) => (
              <p key={`privacy-${idx}`} className={`mb-1.5 last:mb-0 ${line === "" ? "h-2" : ""}`}>
                {line}
              </p>
            ))}
            <hr className="my-4 border-border" />
            {aiNoticeText.split("\n").map((line, idx) => (
              <p key={`ai-${idx}`} className={`mb-1.5 last:mb-0 ${line === "" ? "h-2" : ""}`}>
                {line}
              </p>
            ))}
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-border bg-muted/20 p-4">
            <label className="flex items-start gap-3 text-sm text-foreground cursor-pointer">
              <Checkbox
                ...
                className="mt-0.5"
              />
              <span>I acknowledge and accept the <strong>Terms of Service</strong>.</span>
            </label>
            <label className="flex items-start gap-3 text-sm text-foreground cursor-pointer">
              <Checkbox
                ...
                className="mt-0.5"
              />
              <span>I acknowledge and accept the <strong>Privacy Policy</strong> and data protection terms.</span>
            </label>
            <label className="flex items-start gap-3 text-sm text-foreground cursor-pointer">
              <Checkbox
                checked={acceptedAiNotice}
                onCheckedChange={(checked) => setAcceptedAiNotice(Boolean(checked))}
                className="mt-0.5"
              />
              <span>I understand AI outputs are <strong>informational only</strong> and not legal advice.</span>
            </label>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            ...
            <Button
              ...
              className="bg-[#5B5AF7] hover:bg-[#4A49E8] text-white"
            >
